...
class DragWidget:
    ...
    def add_point(self, click, x, y):
        ...
        if click:
            ...
            predictor =dlib.shape_predictor('./viz/shape_predictor_68_face_landmarks.dat')
            image = cv2.imread("./viz/image.png")
            gray=cv2.cvtColor(image,cv2.COLOR_BGR2GRAY)
            faces=detector(gray)
            for face in faces:
                landmarks=predictor(gray,face)
                left_mouth_corner = [int(landmarks.part(48).y), int(landmarks.part(48).x)]
                right_mouth_corner = [int(landmarks.part(54).y), int(landmarks.part(54).x)]
                self.points.append(left_mouth_corner)
                self.points.append(right_mouth_corner)
                self.targets.append([int(landmarks.part(48).y), int(landmarks.part(48).x)-7])
                self.targets.append([int(landmarks.part(54).y), int(landmarks.part(54).x)+7])
